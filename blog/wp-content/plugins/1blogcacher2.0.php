...
IF (!function_exists("obc_options_page")){
	FUNCTION obc_options_page(){
		...
		IF (basename(__FILE__) != "1blogcacher2.0.php"){
			...
			echo "<td><p>{$get_files["files"]} files aved in your cache directory (".round($get_files["size"]/(1024),2)." KB = ".round($get_files["size"]/(1024*1024),2)." MB)";
			...
			echo "<td><p>{$get_files["expired_files"]} expired files aved in your cache directory (".round($get_files["expired_size"]/(1024),2)." KB = ".round($get_files["expired_size"]/(1024*1024),2)." MB)";
