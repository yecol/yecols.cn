     ...
     <?php } else { ?>
      ...
      <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform" name="tcommentform">
        ...
        <?php if ($user_ID) : ?>
  		...
  		<p><?php _e('Logged in as', 'fusion'); ?> <a href="<?php echo get_option('siteurl'); ?>/wp-admin/profile.php"><strong><?php echo $user_identity; ?></strong></a>. <a href="<?php echo $logout_link; ?>" title="<?php _e('Log out of this account', 'fusion'); ?>"><?php _e('Logout &raquo;', 'fusion'); ?></a></p>
   	  <?php else : ?>
