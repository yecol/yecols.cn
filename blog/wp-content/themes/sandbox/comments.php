...
<?php $sandbox_comment_alt = 0 ?>
				...
				<div id="comments-list" class="comments">
					<?php printf($comment_count > 1 ? __('<span class=sidebarHeadFont>%d Comments</span>', 'sandbox') : __('<span class=sidebarHeadFont>One Comment</span> ', 'sandbox'), $comment_count) ?>
...
<?php if ($comment->comment_approved == '0') _e("\t\t\t\t\t<span class='unapproved'></span>\n", 'sandbox') ?>
							<?php comment_text() ?>
						</li>
...
<?php $sandbox_comment_alt = 0 ?>
				...
				<div id="trackbacks-list" class="comments">
					<span class=sidebarHeadFont><?php printf($ping_count > 1 ? __('<span>%d</span> Trackbacks', 'sandbox') : __('<span>One</span> Trackback', 'sandbox'), $ping_count) ?></span>
...
<?php $req = get_option('require_name_email'); // Checks if fields are required. Thanks, Adam. ;-) ?>
				...
				<div id="respond">
					<div class="postacomment"><?php _e( 'Post a Comment', 'sandbox' ) ?></div>
...
<?php else : ?>
							...
							<p id="comment-notes"><?php _e( '你的邮箱不会回显在本页面，评论内容星号标记项为必填项。<br>Your email is <em>never</em> shared.', 'sandbox' ) ?> <?php if ($req) _e( 'Required fields are marked <span class="required">*</span>', 'sandbox' ) ?></p>

							
							<div class="form-input"><input id="author" name="author" class="text<?php if ($req) echo ' required'; ?>" type="text" value="<?php echo $comment_author ?>" size="30" maxlength="50" tabindex="3" /><div class="form-label"><label for="author"><?php _e( '昵称/Name', 'sandbox' ) ?></label> <?php if ($req) _e( '<span class="required">*</span>', 'sandbox' ) ?></div></div>

							
							<div class="form-input"><input id="email" name="email" class="text<?php if ($req) echo ' required'; ?>" type="text" value="<?php echo $comment_author_email ?>" size="30" maxlength="50" tabindex="4" /><div class="form-label"><label for="email"><?php _e( '邮箱/Email', 'sandbox' ) ?></label> <?php if ($req) _e( '<span class="required">*</span>', 'sandbox' ) ?></div></div>

							
							<div class="form-input"><input id="url" name="url" class="text" type="text" value="<?php echo $comment_author_url ?>" size="30" maxlength="50" tabindex="5" /><div class="form-label"><label for="url"><?php _e( '网站/Website', 'sandbox' ) ?></label></div></div>

<?php endif // REFERENCE: * if ( $user_ID ) ?>

							<div class="form-textarea"><textarea id="comment" name="comment" class="text required" cols="45" rows="8" tabindex="6"></textarea></div>

							<div class="form-submit"><input id="submit" name="submit" class="button" type="submit" value="<?php _e( '发表评论', 'sandbox' ) ?>" tabindex="7" /><input type="hidden" name="comment_post_ID" value="<?php echo $id ?>" /></div>
